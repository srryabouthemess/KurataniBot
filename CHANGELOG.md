...
- **`/scorewipe player: mode: reason: [list:] [score:]` — apaga UM score.** [`scorewipe.js`](src/commands/scorewipe.js), [`daycoreAdmin.js`](src/daycoreAdmin.js), [`banchoPyApi.js`](src/osu/banchoPyApi.js)
  ...
  - **O id do score vem de um menu.** O id está na URL da página do score no site (`/scores/<id>`), mas chegar até ela exige passar pela leaderboard do mapa certo, e os embeds do bot não trazem o id. O comando lista as dez melhores plays do alvo (ou as dez mais recentes, com `list:recent`), com mapa, pp, acurácia, mods e data em cada linha. O `score:` continua lá para quem já tem o id.
...
- **No `/link`, "qual servidor" e "VN ou RX" viraram duas perguntas.** [`link.js`](src/commands/link.js), [`userLink.js`](src/userLink.js), [`servers.js`](src/servers.js), [`db/users.js`](src/db/users.js), [`db/schema.js`](src/db/schema.js), [`db/migrations.js`](src/db/migrations.js)
...
- **O projeto passou a cumprir as licenças que usa.** [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md) (novo), [`gerar-notices.js`](src/gerar-notices.js) (novo), `npm run notices`
  ...
  - **Duas dependências o npm não enxerga, e elas entram à mão**: o `akatsuki-pp-py` (MIT), que o [`pp_calc.py`](src/pp_calc.py) importa para o PP do Relax, e os **ícones de grade** em [`assets/emojis/`](assets/emojis), que são da skin oficial do osu!.
  - **Os ícones trazem a restrição que vale conhecer.** O código do osu! é MIT, os recursos do jogo **não**: eles são [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/), que pede atribuição e proíbe uso comercial. Enquanto o bot for gratuito eles ficam; se um dia cobrar, saem — e a saída não quebra nada, porque sem os arquivos a grade volta a ser texto. O [README da pasta](assets/emojis/README.md) passou a dizer isso onde alguém que for trocar os arquivos vai ler.
