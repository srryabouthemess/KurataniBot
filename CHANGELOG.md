...
- **Os grupos do servidor entraram nos dois comandos, do jeito que o site usa.** [`banchoPyApi.js`](src/osu/banchoPyApi.js), [`leaderboard.js`](src/commands/leaderboard.js), [`topscores.js`](src/commands/topscores.js)
  ...
  - Custo: uma página de perfil por **jogador** (não por play), com cache de meia hora — a lista inteira do Daycore é de menos de dez pessoas. No `/leaderboard` são os dez da página exibida, e o `clan_tag` (`[flau]`) vem de graça, já dentro da resposta do ranking.
  - **Na tela do `/leaderboard` só aparecem ✅ Legit e ❌ Cheating.** Exibir os oito virava fileira de emoji — o kyou sozinho carregava seis (🐾✍🛡️✅🛠️🌟), empurrando o nick e competindo com a bandeira. Estes dois ficam porque são os únicos que falam do **número ao lado**: se aquele pp é confiável. Cargo (Nominator, Moderator, Developer, Supporter) e apelido interno (puppy, Fuquila) são do perfil da pessoa, não da posição dela. O que o `/topscores` **esconde** continua sendo Cheating e Fuquila, porque lá a regra é a do site e não a da tela.
