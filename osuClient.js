...
function normalizeUserDaycore(playerData, statsData, mode) {
  ...
  return {
    ...
    is_online: false,
    statistics: {
      global_rank: statsData?.rank ?? null,
      country_rank: statsData?.country_rank ?? null,
      pp: parseFloat(statsData?.pp ?? 0),
      hit_accuracy: parseFloat(statsData?.acc ?? 0),
      level: { current: 1 },
      maximum_combo: statsData?.max_combo ?? 0,
      play_count: statsData?.plays ?? 0,
    },
    _private: true,
  };
}
...
// ─── FC PP via Python (Relax / akatsuki-pp-py) ───────────────────────────────
/**
...
async function getUser(username, mode = DEFAULT_MODE) {
  ...
  return normalizeUserDaycore(playerData, statsData, mode);
}
...
async function getBestScores(userId, limit = 10, mode = DEFAULT_MODE) {
  ...
  return enrichScores(res.scores ?? []);
}
